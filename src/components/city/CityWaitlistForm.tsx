...
const CityWaitlistForm = ({ city }: CityWaitlistFormProps) => {
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="max-w-md mx-auto">
          <div className="text-center mb-8">
            ...
            <div className="flex items-center justify-center gap-2 mt-4 mb-2">
              <div className="flex -space-x-2">
                ...
                <div className="w-8 h-8 rounded-full bg-primary/10 border-2 border-background flex items-center justify-center text-xs font-bold text-primary">S</div>
                <div className="w-8 h-8 rounded-full bg-primary/20 border-2 border-background flex items-center justify-center text-xs font-bold text-primary">T</div>
                <div className="w-8 h-8 rounded-full bg-primary/30 border-2 border-background flex items-center justify-center text-xs font-bold text-primary">J</div>
              </div>
              <span className="text-sm font-medium text-primary">{city.waitlistCount} Eltern warten bereits</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="bg-card p-6 md:p-8 rounded-2xl shadow-card space-y-5">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="Dein Vorname (z.B. Sarah)" value={formData.name} onChange={(e) => { setFormData({ ...formData, name: e.target.value }); setErrors((prev) => ({ ...prev, name: "" })); }} className={errors.name ? "border-destructive" : ""} />
              {errors.name && <p className="text-sm text-destructive">{errors.name}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="email">E-Mail</Label>
              <Input id="email" type="email" placeholder="deine.email@adresse.de" value={formData.email} onChange={(e) => { setFormData({ ...formData, email: e.target.value }); setErrors((prev) => ({ ...prev, email: "" })); }} className={errors.email ? "border-destructive" : ""} />
              {errors.email && <p className="text-sm text-destructive">{errors.email}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="plz">PLZ / Stadtteil</Label>
              <Input id="plz" placeholder="PLZ oder Stadtteil" value={formData.plz} onChange={(e) => { setFormData({ ...formData, plz: e.target.value }); setErrors((prev) => ({ ...prev, plz: "" })); }} className={errors.plz ? "border-destructive" : ""} />
...
export default CityWaitlistForm;
